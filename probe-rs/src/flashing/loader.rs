...
impl FlashLoader {
    ...
    pub fn commit(
        ...
        if let BootInfo::FromRam { cores_to_reset, .. } = self.boot_info() {
            ...
            // clear state. Normally, flash algorithm loader performs reset and halt - does not
            // happen here. Some targets are expected to already be reset by other tooling before
            // RAM flashing runs, in which case the target description disables this reset and the
            // core is only halted.
